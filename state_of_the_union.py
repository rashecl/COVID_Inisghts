...
usPlot = figure(title="Cases of Coronavirus", tools=us_TOOLS,
                ...
                tooltips=[("Current cases", "@cases"), ('State', '@state')],
...
statePlot = figure(title="Coronavirus map", tools=state_TOOLS,
                   x_axis_location=None, y_axis_location=None,
                   tooltips=[('Name', '@name'), ("Current cases", "@cases"), ('State', '@state')],
...
dailyPlot.add_tools(
    HoverTool(renderers=[new_testResults], tooltips=[("new_testResults", "@new_testResults"), ("date", "@date")]))
dailyPlot.add_tools(
    HoverTool(renderers=[new_positive_NYT], tooltips=[("new_positive", "@new_positive"), ("date", "@date")]))
...
new_deaths_CT = dailyCriticalPlot.line('time', 'new_deaths', source=dailyData_CT, line_color='black', line_width=2,
...
dailyCriticalPlot.add_tools(
    HoverTool(renderers=[new_deaths_NYT], tooltips=[("new_deaths", "@new_deaths"), ("date", "@date")]))
dailyCriticalPlot.add_tools(HoverTool(renderers=[current_hospitalized],
                                      tooltips=[("current_hospitalized", "@current_hospitalized"), ("date", "@date")]))
dailyCriticalPlot.add_tools(
    HoverTool(renderers=[current_ICU], tooltips=[("current_ICU", "@current_ICU"), ("date", "@date")]))
...
def updateState():
    ...
    cumulPlot.title.text = stateBorders[state]['name']
...
def sourceUSdata():
    ...
    cumulativeData_NYT.data = dict(
        ...
        total_deaths=NYTdf['death'],
        source=NYTdf['source'])
    cumulPlot.title.text = 'United States: Cumulative data'
    return

def sourceCountyData():
    global stateCountyDF, county
    NYTdf = stateCountyDF
    dailyData_CT.data = dict(
        time=[],
        date=[],
...
def state_tap(attr, old, new):
    ...
    if len(new) == 0:
        ...
        if len(stateCountyDF) == 0:
            ...
            pass
        sourceCountyData()
        cumulPlot.title.text = county + ": Cumulative plots"
